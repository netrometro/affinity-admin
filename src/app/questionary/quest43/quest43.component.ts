...
export class Quest43Component implements OnInit {  // <= numero
  ...
  ngOnInit() {
    let a = this.questionaryService.getAnswers(43); // <= numero
    this.upAnswer = a[0];
    this.downAnswer = a[1];
  }
  ...
  responder(posicao: number, resposta: number) {
    ...
    this.questionaryService.save(43, posicao, resposta);  // <= numero
  }
}