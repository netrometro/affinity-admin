...
export class Quest32Component implements OnInit {  // <= numero
  ...
  ngOnInit() {
    let a = this.questionaryService.getAnswers(32); // <= numero
    this.upAnswer = a[0];
    this.downAnswer = a[1];
  }
  ...
  responder(posicao: number, resposta: number) {
    ...
    this.questionaryService.save(32, posicao, resposta);  // <= numero
  }
}